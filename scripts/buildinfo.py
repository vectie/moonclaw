...
def extract_version_string_from_file(buildinfo_mbt):
    """Extract version string from existing buildinfo.mbt file."""
    try:
        with open(buildinfo_mbt, "r") as f:
            content = f.read()
            match = re.search(r'let version_string : String = "([^"]+)"', content)
            if match:
                return match.group(1)
...
def generate_buildinfo():
    ...
    target_version_string = f"{version}+{commit_count}.{commit_sha}"
    existing_version_string = extract_version_string_from_file(version_string_mbt)
    if existing_version_string == target_version_string:
        print(
            f"Buildinfo already up-to-date (commit {commit_sha}), skipping regeneration"
        )
        return
    print(
        f"Updating buildinfo from {existing_version_string} to {target_version_string}"
    )
...
let version_string : String = "{target_version_string}"
"""
