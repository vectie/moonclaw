...
- **Evidence:** New scalar/spool and real-HTTP coverage passes 5/5 over torn mixed v1/v2 input, foreign-session rejection, exact JSONL/SSE byte parity, huge cursors, non-event wakeups, and 5,000 records. The pre-existing stream suite remains 9/9, the daemon suite is 300/300, and the full native repository suite is 1361/1361. Native daemon check reports 0 errors; `moon info`, `moon fmt`, and `git diff --check` succeed with no generated-interface diff.
- **Open boundaries:** Full session/conversation/listing projections still contain aggregate journal-derived arrays, and snapshot loading retains its separate explicit 2 GiB compatibility limit. Temporary spool creation can fail on disk capacity or I/O errors. Multiprocess delivery/append, crash/failpoint, and real-Windows open-handle/cleanup lanes remain unproven. The policy-unbounded goal-runtime contract is also not yet the live supervisor: the legacy HTTP goal parser, runtime-loop `max_turns`, model-planner `max_steps`, and process-local service singleton are the next migration.
