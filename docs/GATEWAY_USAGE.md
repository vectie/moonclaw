...
```bash
moon run cmd/main -- gateway start --home /path/to/MoonSuiteRoot --cwd /path/to/workspace --port 18124
moon run cmd/main -- gateway health --url http://localhost:18124 --home /path/to/MoonSuiteRoot --cwd /path/to/workspace
moon run cmd/main -- gateway channels --url http://localhost:18124 --home /path/to/MoonSuiteRoot --cwd /path/to/workspace
```
...
moonclaw gateway start --cwd /path/to/project --home /path/to/MoonSuiteRoot
```
...
- normal non-reply chat still falls through to the usual MoonClaw conversation
  path
- normal channel chat uses the configured primary model from `/path/to/MoonSuiteRoot/.moonsuite/products/moonclaw/moonclaw.json`; stale bare session model ids are normalized instead of overriding that config
